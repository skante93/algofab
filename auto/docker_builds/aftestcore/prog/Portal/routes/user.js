...
module.exports = function(SG) {
	...
	*/
	...
	router.get('/bugreports', function(req, res){
		//
		BugReports.find({ addressedTo : req.session.user._id}).populate(["emittedBy", { path : "algo_version", populate : { path : "meta" } }]).exec(function(err, reports){
			//

			res.render('bugreports', { title : "Bug Reports", activeHeadersComp : 'user', user : req.session.user, reports : reports, err : err });
		});
	});
