	...
	*/
	...
	var removeInfra = function(id, cb){
		...
		Algos.findById(id).populate({
			...
			var kube_objects_to_delete = algo.deployment.kubernetes;
	...
	var createInfra = function(id, io, cb){
		...
		.then(algo => {
			...
			console.log("Now building the "+algo.deployment.kubernetes.length+" objects");
			
			var kube_objects_to_create = Array.from(algo.deployment.kubernetes);
			...
			(function create_kube_objects(callback){
				...
				if (kube_objects_to_create.length == 0){
					...
					recordSpecs(algo.meta.author.username, algo.meta.title, algo.version, algo.deployment.kubernetes);
					return callback();
				}
