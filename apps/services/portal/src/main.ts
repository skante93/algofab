...
$.fn.extend({
	...
	styleResource: function(){
		...
		return this.each(function(index, e){
			...
			var content = $('<div><div class="resource_logo">Logo</div><a routerLink="/resources/'+id+'"><button class="resource_redirect_btn"> More... </button></div></a>');
	...
	bannerIMG: function(){
		$('[banner-img]').each(function(index, e){
			//alert('Hi there');
			$(e).css({
	...
	customStyles: function(){
		$("[my-custom-styles]").each(function(index, e){
			...
			try{
				...
				console.error("Could not apply custom style because : ", e);
			}
		});
	}
});
