...
var agreementsSchema = new Schema(
	{
		...
		content: {
			media_type: {type: String, required: true, enum: ["html", "file"]},
			html_content: String,
			file_content: Buffer
		},
...
var usersSchema = new Schema(
	{ // Schema
		profile: {
			...
			firstname: String,
			lastname: String,
			//status: {type: String, enum: ["user", "admin", "super"]},
			photo: { type: Schema.Types.ObjectId, ref : 'Photos' }
			//photo: { content_type: String, buffer: Buffer }
		},
		status: {type: String, enum: ["user", "admin", "super"], default: "user"},
...
var resourcesSchema = new Schema (
	{
		metadata: {
			...
			type: {type: String, enum: ['ai_model', 'notebook', 'dataset', 'docker', 'executable', 'service', 'library']},
