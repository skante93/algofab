...
class RestResponse {
    ...
    reply(req, res, filter){
        if (this.warning != null){
            //
            res.status(this.statusCode).json({ type: "warning", response: filter ? filter(this.response) : this.response, warning: this.warning });
        }
        else if (this.statusCode >= 200 && this.statusCode < 300){ //success
            //
            res.status(this.statusCode).json({ type: "success", response: filter ? filter(this.response) : this.response });
        }
        else {
...
const createJWT = (options)=> {
    var tkn = jwt.encode(options, settings.api.jwt_secret);
    return tkn;
}

const decodeJWT = (tkn)=> {
    return jwt.decode(tkn, settings.api.jwt_secret);
}
