...
var m_User = CONFIG.mongo.model('User'), 
        m_AlgosMeta = CONFIG.mongo.model('AlgosMeta'), 
            m_Algos = CONFIG.mongo.model('Algos'), 
                m_Subscriptions = CONFIG.mongo.model('Subscriptions');
                    m_Token = CONFIG.mongo.model('Token'); 
                        m_Context = CONFIG.mongo.model('Context');
                            m_Requests = CONFIG.mongo.model('Requests');
...
app.use(function(req, res, next){
    ...
    //var r = new m_Requests({}).save(function(err, result){
        ...
        var t1 = Date.now(), stream = strm(req, res), output = request[req.method.toLowerCase()](target);
        stream.pipe(output);
        output.on('response', function(response){
            console.log("Got response, code : "+response.statusCode+", type : "+response.headers['content-type']);
            var received = "";
            response.on('end', function(p){
                //
                console.log("on end : COOL : "+p);
                if(/application\/json/.test(response.headers['content-type'])){
                    received = JSON.parse(received);
                    console.log(util.inspect(received));
                }
                afterAlgoResponse(null, t1, req, res, res.locals.algo_response, response, received);
            })
        ...
        .on('error', function(error){
            ...
            afterAlgoResponse(error, t1, req, res, res.locals.algo_response);
        });
        /*function(error, response, body){
            ...
            //console.log("Connexion closed : "+util.inspect( body));
            
            var t2 = Date.now(), delta = (t2-result.beginningDate.getTime())/1000;
            //console.log("beginningDate : "+result.beginningDate+", t2 : "+new Date(t2).toString()+",  delta : "+delta);
            var history = {
                date : result.beginningDate,
                duration : delta,
            ...
            result.update({ $set : {
                response : { 
                    status : (statusCode == null)? 500 : statusCode, 
                    body : (statusCode == null)? error.toString() : body 
                },
                duration : delta,
                is_responding : false
            }}).exec(function(err){
        ...
        */
        /*
        var output = request[req.method.toLowerCase()](target);
        req.pipe(output);
        output.on('response', function(response){
            console.log("Got response");
            response.on('end', function(body){
                //
                //var duration = Date.now() - result.beginningDate;
                console.log("Connexion closed : "+util.inspect( body));
                var t2 = Date.now(), delta = (t2-result.beginningDate)/1000;
