...
user.find({username : 'user'}).exec(function(err, res){
  console.log(err || res);
  res.username = "toto";
  res.save();
});
...
var m = require("./config").mongo.model("Context");
var history = {
  date : Date.now(),
  duration : 10,
  algo : "59df44a53cd28e1890d060af"
}

m.update({ _id : "59de9e9e5166ae187a929ca6" }, {
    $push : {
        history : {
            $each : [history]
        }
