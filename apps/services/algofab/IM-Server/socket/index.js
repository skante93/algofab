	...
	io.on('connection', function(socket){
		...
		socket.on('states', function(socket_id, id){
			...
			Algos.findById(id).populate({
				...
				var watchers = [];
				var kubes = (typeof algo.deployment.k8s !== 'undefined' && algo.deployment.k8s != null)? JSON.parse(algo.deployment.k8s) : algo.deployment.kubernetes;
				kubes.forEach(function(kubeObject, index){
