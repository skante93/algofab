...
module.exports = function(SG){
	...
	var im_socket = require('socket.io-client')(global.settings.IM_PROTOCOL + global.settings.IM_ADDR +":"+global.settings.IM_PORT);
	...
	var im_client = function(main_socket){
		...
		this.describe = function(socket_id, kind, name, result){
			if(socket_id == main_socket.id) {
				console.log(' PORTAL <<<<<<<< IM | <DESCRIBE> | KIND : '+kind+', NAME : '+name);
				console.log("result : "+result);
				main_socket.emit('describe', kind, name, result);
			}
		}
		
		this.full_spec = function(socket_id, kind, name, result){
			//console.log(socket_id+' : PORTAL <<<<<<<< IM | <DESCRIBE> | KIND : '+kind+', NAME : '+name);
				
			if(socket_id == main_socket.id) {
				console.log(' PORTAL <<<<<<<< IM | <FULL SPEC> | KIND : '+kind+', NAME : '+name);
				console.log("result : "+result);
				main_socket.emit('full spec', kind, name, result);
			}
		}
		...
		this.edit_version = function(socket_id, status, vid, message){
			//console.log(socket_id+' : PORTAL <<<<<<<< IM | <DESCRIBE> | KIND : '+kind+', NAME : '+name);
				
			if(socket_id == main_socket.id) {
				console.log(' PORTAL <<<<<<<< IM | <EDIT VERSION> | STATUS : '+status+', ALGOID : '+vid+", MESSAGE : "+message);
				main_socket.emit('edit version', status, vid, message);
			}
		}

		this.edit_version_progress = function(socket_id, vid, message){
			//console.log(socket_id+' : PORTAL <<<<<<<< IM | <DESCRIBE> | KIND : '+kind+', NAME : '+name);
				
			if(socket_id == main_socket.id) {
				console.log(' PORTAL <<<<<<<< IM | <EDIT VERSION PROGRESS> | ALGOID : '+vid+", MESSAGE : "+message);
				main_socket.emit('edit version progress', vid, message);
			}
		}
		...
		im_socket.on('describe', this.describe);
		im_socket.on('full spec', this.full_spec);
		...
		this.clean = function(){
			...
			im_socket.removeListener('describe', this.describe);
			im_socket.removeListener('full spec', this.full_spec);
	...
	return function(server, sessionStore) {
		...
		socket_io.on('connection', function(socket){
			...
			socket.on('room', function(room){
				...
				socket.emit("Hello", SG.mongo.Types.ObjectId().toString());
			});
			...
			socket.on('describe', function(kind, name){
				console.log(' PORTAL >>>>>>>> IM | <DESCRIBE> | KIND : '+kind+', NAME : '+name);
				im_socket.emit('describe', socket.id, kind, name, session.user.username);
			});
			
			socket.on('full spec', function(kind, name){
				console.log(' PORTAL >>>>>>>> IM | <FULL SPEC> | KIND : '+kind+', NAME : '+name);
				//console.log('\nSG SOCKET ON full description : "'+kind+'" '+name);
				im_socket.emit('full spec', socket.id, kind, name, session.user.username);
			});
			...
			socket.on('create algo', function(title, description, keywords, logo){
				...
				AlgosMeta.findOne({ title : title }).exec(function(err, algo){
					...
					n_algo._id = SG.mongo.Types.ObjectId().toString();
				...
				AlgosMeta.update({ _id : algoMetaID, author : session.user._id}, { $set : { delete_date : Date.now()+global.settings.TIME_TO_WAIT_BEFORE_DELETING_ALGO } }).exec(function(err, u){
					...
					RMV_ALGO_TIMERS[algoMetaID] = setTimeout(function(){
						...
						AlgosMeta.findById(algoMetaID).populate('author versions').exec(function(err, algo){
							...
							//Hide versions
							Algos.update({ meta: algoMetaID }, { $set : { hidden : true } }).exec(function(errHide){
								if(errHide)
									return console.log("Could not hide the versions : "+errHide);
								console.log("Versions successfully hidden");
							});
							
							(function removeVers(){
								var v = algo.versions.splice(0,1)[0];
								
								restler.post(global.settings.IM_PROTOCOL + global.settings.IM_ADDR +':'+ global.settings.IM_PORT+ "/remove-algo-version", 
				                	{ data : {_id : v._id.toString() } }
			...
			socket.on('edit version', function(vid, contents){
				console.log(' PORTAL >>>>>>>> IM | <EDIT VERSION> | ALGOID : '+vid+', version : '+contents.version);
				im_socket.emit('edit version', socket.id, vid, contents, session.user.username);
			});
			...
			socket.on('create version', function(algoMetaID, manifest){
				...
				SG.utils.dao.algo.version.analyseManifest(algoMetaID, { content : manifest }, undefined, session.user.username, function(err, newAlgo){
		      		if(err){
		      			console.log("Err: "+err);
	      			...
	      			Algos.collection.insert([newAlgo], {}, function(s_err, docs){
						if(s_err){
							console.log("DB error : "+s_err);
	      					return socket.emit("create version progress", "DB error");
	      				}
			...
			socket.on('remove version', function(vid){
				console.log(' PORTAL >>>>>>>> IM | <REMOVE VERSION> | ID : '+vid);
				
				Algos.findById(vid).populate(
					{
						path : "meta",
						populate : {
							path : "author"
					...
					restler.post(global.settings.IM_PROTOCOL + global.settings.IM_ADDR +':'+ global.settings.IM_PORT+ "/remove-algo-version", 
						...
						Algos.remove({ _id : vid }).exec(function(err){
							if(err) {
			...
			socket.on('toggle hidden', function(vid){
				console.log(' <TOGGLE HIDDEN> | ALGOID : '+vid);
				Algos.findById(vid).populate(
					{
						path : "meta",
						populate : {
