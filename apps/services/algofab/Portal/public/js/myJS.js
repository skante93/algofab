...
var traceCreateOrUpdateArticleSocket = function(op){
    //
    var m = $(`
        <div class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title"> ${(op && op == "create")?"Create ":"Edit the "}  Article</h4>
                    </div>
                    <div class="modal-body">
                        <div id="s-trace-update-algo" class="row"></div>
                    </div>
    ...
    var onCreateOrUpdateArticle = function(status, msg){
        var msg = $(`<div class="alert ${(status == "success")?"alert-success":"alert-danger"}">${msg}</div>`);
        m.find("#s-trace-update-algo").append(msg);
        m.find("#s-trace-update-algo").append( $('<div></div>').append( $(`<button class="btn btn-primary pull-right">Refresh</button>`).click(function(){window.location.reload();}) ) );
    }
    var onCreateOrUpdateArticleProgress = function(msg){
        //
        console.log('on : msg = '+msg);
    ...
    if(op && op == "create"){
        socket.on("create article", onCreateOrUpdateArticle);
        socket.on("create article progress", onCreateOrUpdateArticleProgress);
    }
    else{
        socket.on("update article", onCreateOrUpdateArticle);
        socket.on("update article progress", onCreateOrUpdateArticleProgress);
    }
...
var updateAlgoModal = function(id, title, description, tags){
    ...
    console.log("############### WYSIWYG mod.find('#description'): ", mod.find('#description'));
    ...
    setTimeout(function(){
        tinymce.init({
            selector: mod.find('textarea#description'),
            height: 500,
            menubar: false,
            plugins: [
...
var shareVersionContent = function(version){
    var content;

    if (typeof version.deployment.k8s !== 'undefined' && version.deployment.k8s != null){
        //
        content = $(`
            <div>
                <p>To share this algorithm, make sure you have <a href="https://kubernetes.io">Kubernetes</a> alreay installed.</p>
                <p>Once your kubernetes cluster is ready, copy the following content in to file (let's call it share.json) :</p>
                <p>
                    <pre>${JSON.parse(version.deployment.k8s).map(e=> JSON.stringify(e, null, 2)).join('\n\n')}</pre>
                </p>
                <p>
                    After your create your share.json, launch the following Kubernetes command on your master:
                </p>
                <p><pre>kubectl create -f share.json</pre></p>.
                <hr>
        ...
        if (version.deployment.kubernetes.filter(e => e.kind == "Pod").length == 1 && version.deployment.kubernetes.filter(e => e.kind == "Pod")[0].spec.containers.length == 1){
            //
            var cont = version.deployment.kubernetes.filter(e => e.kind == "Pod")[0].spec.containers[0];

            var cmd = cont.command ? cont.command : 'Not specified (you can skip it when launching the container)';
            var args = cont.args ? cont.args : 'Not specified (you can skip it when launching the container)';
            var env = cont.environment ? cont.environment : 'Not specified (you can skip it when launching the container)';

            content = $(`
                <div>
                    <p>To share this algorithm, make sure you have <a href="https://docker.io">Docker</a> alreay installed.</p>
                    <p>Once your Docker is running, launch the following command on your docker host to get the container:</p>
                    <p>
                        <pre>docker pull ${version.deployment.kubernetes.filter(e => e.kind == "Pod")[0].spec.containers[0].image}</pre>
                    </p>
                    <hr>
                    <p>
                        <b><em>Note:</em></b> The options from the kubernetes spec are:
                        <ul>
                            <li>Command: <b>${cmd}</b> </li>
                            <li>Args: <b>${args}</b> </li>
            ...
            var demo_access, api_access;
            if ("http" in version.deployment.services.demo){
                demo_access = `http://<SVC:${version.deployment.services.demo.http.svc}>`;
                if ("port" in version.deployment.services.demo.http){
                    demo_access += `:<PORT:${version.deployment.services.demo.http.port}`;
                }
            }
            else{
                demo_access = `https://<SVC:${version.deployment.services.demo.http.svc}>`;
                if ("port" in version.deployment.services.demo.http){
                    demo_access += `:<PORT:${version.deployment.services.demo.http.port}`;
                }
            }
            
            if ("http" in version.deployment.services.api){
                api_access = `http://<SVC:${version.deployment.services.api.http.svc}>`;
                if ("port" in version.deployment.services.api.http){
                    demo_access += `:<PORT:${version.deployment.services.api.http.port}`;
                }
            }
            else{
                api_access = `https://<SVC:${version.deployment.services.api.http.svc}>`;
                if ("port" in version.deployment.services.api.http){
                    demo_access += `:<PORT:${version.deployment.services.api.http.port}`;
                }
            }
            ...
            content = $(`
                <div>
                    ...
                    <p>Once your kubernetes cluster is ready, copy the following content in to file (let's call it share.json) :</p>
                    <p>
                        <pre>${version.deployment.kubernetes.map(e=> JSON.stringify(e, null, 2)).join('\n\n')}</pre>
                    </p>
                    <p>
                        After your create your share.json, launch the following Kubernetes command on your master:
                    </p>
                    ...
                    <hr>
                    <p>
                        <b><em>Note:</em></b> The accesses are:
                        <ul>
                            <li>Demo: <b>${demo_access}</b> </li>
                            <li>API: <b>${api_access}</b></li>
                        </ul>
                    </p>
                    <p>If you have difficulties accessing the docker image (private images), you should contact the creator of the algorithm (top of this page) and discuss it with him.</p>
                </div>`
            )
        }
    }
...
var versionCollapseContent = function(version, authorID){
	...
	var demo_content = $('<div class="row"></div>').append( $('<a href="/demo/'+version._id+'/" target="_blank" class="btn btn-primary offset-md-5 col-md-2">Show demo</a>') );
 //        .click(function(){
    ...
    var share_content = shareVersionContent(version);
    ...
    var share_tab = $(`<li role="presentation" id="share-${version.version.replace(/\./g, '-')}"><a href="#"> Share </a></li>`).click(function(){
        console.log("Tab BUG");
        ...
        current_display.empty();
        current_display.append(share_content);
    });
...
var versionHandler = function(algoID, authorID, version, mode){
	...
	if(mode == "admin" ){
		...
		var share = $(`<span class="glyphicon glyphicon-share algo-version-option-share ${(version.shareable)?'text-primary':''}" aria-hidden="true" id="share-version-${version.version}" title="Toggle property shareable" vid="${version._id}" ></span>`);
        var remove = $(`<span class="glyphicon glyphicon-trash algo-version-option-remove" aria-hidden="true" id="remove-version-${version.version}" title="Remove this version" vid="${version._id}"></span>`);
        
		var options = $(`<span class="pull-right"></span>`);
		options.append(health).append(edit).append(hide).append(share).append(remove);
        ...
        var share_event_handler = function(status, vers, message){
            if(vers == version.version){
                if(share.hasClass("text-primary")){
                    //
                    share.removeClass("text-primary");
                    //share.addClass("text-warning");
                    share.attr('title', 'Make this version shareable.');
                }
                else {
                    //
                    //share.removeClass("text-warning");
                    share.addClass("text-primary");
                    share.attr('title', 'Make this version unshareable.');
                }
                message = `<pre style="background: black; color: ${(status=="success")?"green":"red"}">This version is now ${(message)? "shareable": "unshareable"}.</pre>`;
                //myModal("Result of Toggle Hidden property on Version "+vers, message, { foreground: "indigo", background : "grey" }, {background : "black"}).modal('show');
                
                notificationMsg("Result of Toggle Shareable property on Version "+vers, 
                    message, 
                    5000,
                    {
                        title : { foreground: "indigo", background : "grey" }, 
        ...
        socket.on('toggle share', share_event_handler);
        share.click(function(){
            socket.emit("toggle share", version._id);
            console.log(`Version ${version.version}: share`);
        });
        ...
        */
		...
		try{
			...
			for(let i=0; i < versions.length; i++){
                ...
                if (typeof versions[i].deployment.k8s !== 'undefined' && versions[i].deployment.k8s != null){
                    versions[i].deployment.kubernetes = JSON.parse(versions[i].deployment.k8s);
                }
