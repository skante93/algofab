...
var getTokenHistoryInIntervarl = function(id_ctxt, interv, callback){
...
var getUserHistory = function(u_id, interv, callback){
    ...
    Token.find({bearer : u_id}).exec(function(err, tkns){
        ...
        (function fetchTknHistory(){
            ...
            getTokenHistoryInIntervarl(id_ctxt, interv, function(err, h){
                if(err) {
...
var createKubernetesNamespace = function(username, cb){
	restler.post(global.settings.IM_PROTOCOL + process.env.IM_SERVICE_HOST + "/namespace", 
		{ 
			data : { name : username } 
		}
...
var new_account = function(username, email, cb){
	User.findOne({ $or :[{username : username }, {email : email}] }).exec(function(err, user){
		...
		Pass.findOne({ username : username }).exec(function(errPass, pass){
			console.log("errPass || pass : " + (errPass || pass) )
			if(pass){
				return cb("The username : \""+username+"\" is no longer available.");
			}
			new Pass({ username : username, email : email }).save(function(saveErr, pass){
				...
				mailer.signupEmail(pass, function(mailError) {
				    if (mailError) {
				        console.log(mailError);
				        cb("Mail error");
						User.remove({ username : username }).exec(function(err){ console.log(err || "User sucessfully removed from users collection")});
						return pass.remove(function(err){ console.log(err || "User sucessfully removed from pass collection")});
				    }
...
var confirm_account = function(passID, fname, lname, passwd, cb){
	Pass.findById(passID).exec(function(err, pass){
		...
		if (process.env.WITH_K8S == "TRUE"){
			createKubernetesNamespace(pass.username, function(err){
				if(err){
					return cb (err);
				}
...
var authenticate = function(username, password, cb){
	...
	User.findOne({username : username}, function(err, data){
		...
		err = ldap.authenticate({username : username, password : password});
		if(err instanceof Error)
			return cb('Username '+username+' not recognized');
		
		err = ldap.retrieve_LDAP_account({username : username, password : password});
		console.log('err : '+util.inspect(err));
		
		var u = {
			username : username,
			password : createHash(password),
			firstname : err.givenName,
			lastname : err.sn,
			email : err.mail
		}

		createKubernetesNamespace(username, function(err){
			if(err){
				return cb (err+" while retrieving account from LDAP server");
			}
			...
			u.save(function(error, user){
				...
				mailer.account_from_LDAP(u, function(errMail){
					if(errMail){
						return console.log("Could not send confirmation");
					}
...
var _delete = function(user, cb){
	//
	
	var ldapDeleteErr =  ldap.delete(user);
	if (ldapDeleteErr instanceof Error){
		console.log("ldapDeleteErr:", ldapDeleteErr);
		return cb(ldapDeleteErr);
	}

	//delete namespace 
	request.delete(global.settings.IM_PROTOCOL + process.env.IM_SERVICE_HOST + "/namespace", {form : { name : user.username }} ,function (error, httpResponse, body) {
		console.log("body : "+util.inspect(body));

		if(error){
			console.log("Body is instance of error");
			return cb("Internal Network error : while deleting namespace on Kubernetes");
    	}
		...
		try{
			...
			if(json.status == "failure"){
				console.log("Status is failure");
    			return cb("Deleting namespace on Kubernetes for user : "+json.message);
        	}
		}
		catch(e){

			console.log("Could not parse");
			return cb("delete namespace on Kubernetes : communication error");
		}
...
module.exports = {
	getTokenHistoryInIntervarl : getTokenHistoryInIntervarl,
	getUserHistory : getUserHistory,
	setUsersPassword : setUsersPassword,
	createKubernetesNamespace : createKubernetesNamespace,
