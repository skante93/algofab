...
var K8S_V1_API = `https://${process.env.KUBERNETES_SERVICE_HOST}:${process.env.KUBERNETES_PORT_443_TCP_PORT}/api/v1`;
var K8S_APPS_V1_API = `https://${process.env.KUBERNETES_SERVICE_HOST}:${process.env.KUBERNETES_PORT_443_TCP_PORT}/apis/apps/v1`;

/*
K8S_V1_API=https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_PORT_443_TCP_PORT/api/v1
TKN=$(cat /var/run/secrets/kubernetes.io/serviceaccount/token)

curl --insecure -H "Authorization: Bearer $TKN" $K8S_V1_API/namespaces/skante/event?fieldSelector=involvedObject.name=hello,involvedObject.kind=Pod,involvedObject.namespace=skante -k
*/

//console.log("API_TOKEN: "+API_TOKEN);
//console.log("K8S_V1_API: "+K8S_V1_API);

var accepted_kinds = ["secrets", "namespaces", "pods", "deployments", "services", "replicationcontrollers", "events", "persistentvolumes", "persistentvolumeclaims"];
...
var kubectl = (ns) => {
	...
	var _get = (spec, fieldSelectors) => {
		...
		return new Promise((resolve, reject)=>{
			...
			var options = {
				url: K8S_V1_API+'/namespaces/'+ns+((kind == 'namespaces')? '': ('/'+kind+'/'+(spec.metadata.name? spec.metadata.name: '' )) )+'?pretty=true&fieldSelector='+fieldSelector,
				method: 'GET',
				headers: {
	...
	var _get_using_labels = (kind, labels)=> {
		...
		return new Promise((resolve, reject)=>{
			...
			var options = {
				url: K8S_V1_API+'/namespaces/'+ns+'/'+kind_+'?labelSelector='+labelSelector+'&pretty=true',
				method: 'GET',
				headers: {
					"Authorization": "Bearer "+API_TOKEN
				}
	...
	var delete_deploy = (spec, options, cb) => {
		request(options, (error, response, body)=>{
			// console.log("error : "+error);
			console.log(`DELETE DEPLOY ${spec.metadata.name}, Status: ${response.statusCode}`);
			// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
			if (error || (response.statusCode != 200 && response.statusCode != 201))
				return cb(error || JSON.parse(body).message)
	...
	var create_deploy = (spec, options, cb) => {
		console.log("OPTIONS : "+JSON.stringify(options, null, 2));
		request(options, (error, response, body)=>{
			// console.log("error : "+error);
			console.log(`CREATE DEPLOY ${spec.metadata.name}, Status: ${response.statusCode}`);
			// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
			if (error || (response.statusCode != 200 && response.statusCode != 201))
				return cb(error || JSON.parse(body).message)
	...
	var delete_pod_group = (labels, cb) => {
		_get_using_labels("Pod", labels).catch( cb ).then( (pod_group_object)=> {
			...
			(function delete_pods_one_by_one(error){
				...
				var pod_option = {
					url: K8S_V1_API+'/namespaces/'+ns+'/pods/'+pod_spec.metadata.name+'?pretty=true',
					method: 'DELETE',
					headers: {
						"Authorization": "Bearer "+API_TOKEN,
	...
	var create_secret = (spec, options, cb) => {
		
		console.log("OPTIONS : "+JSON.stringify(options, null, 2));
		request(options, (error, response, body)=>{
			// console.log("error : "+error);
			console.log(`CREATE SECRET ${spec.metadata.name}, Status: ${response.statusCode}`);
			// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
			if (error || (response.statusCode != 200 && response.statusCode != 201))
				return cb(error || JSON.parse(body).message)
			cb(null, JSON.parse(body));
	...
	var delete_secret = (spec, options, cb) => {
		
		console.log("OPTIONS : "+JSON.stringify(options, null, 2));
		request(options, (error, response, body)=>{
			// console.log("error : "+error);
			console.log(`DELETE SECRET ${spec.metadata.name}, Status: ${response.statusCode}`);
			// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
			if (error || (response.statusCode != 200 && response.statusCode != 201))
				return cb(error || JSON.parse(body).message)
			cb();
	...
	var create_pv = (spec, options, cb) => {
		console.log("OPTIONS : "+JSON.stringify(options, null, 2));
		request(options, (error, response, body)=>{
			// console.log("error : "+error);
			console.log(`CREATE PV ${spec.metadata.name}, Status: ${response.statusCode}`);
			// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
			if (error || (response.statusCode != 200 && response.statusCode != 201))
				return cb(error || JSON.parse(body).message)
			cb(null, JSON.parse(body));
	...
	var delete_pv = (spec, options, cb) => {
		
		console.log("OPTIONS : "+JSON.stringify(options, null, 2));
		request(options, (error, response, body)=>{
			// console.log("error : "+error);
			console.log(`DELETE PV ${spec.metadata.name}, Status: ${response.statusCode}`);
			// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
			if (error || (response.statusCode != 200 && response.statusCode != 201))
				return cb(error || JSON.parse(body).message)
			cb();
	...
	var create_pvc = (spec, options, cb) => {
		console.log("OPTIONS : "+JSON.stringify(options, null, 2));
		request(options, (error, response, body)=>{
			// console.log("error : "+error);
			console.log(`CREATE PVC ${spec.metadata.name}, Status: ${response.statusCode}`);
			// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
			if (error || (response.statusCode != 200 && response.statusCode != 201))
				return cb(error || JSON.parse(body).message)
			cb(null, JSON.parse(body));
	...
	var delete_pvc = (spec, options, cb) => {
		
		console.log("OPTIONS : "+JSON.stringify(options, null, 2));
		request(options, (error, response, body)=>{
			// console.log("error : "+error);
			console.log(`DELETE PVC ${spec.metadata.name}, Status: ${response.statusCode}`);
			// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
			if (error || (response.statusCode != 200 && response.statusCode != 201))
				return cb(error || JSON.parse(body).message)
			cb();
	...
	var _create = (spec)=>{
		return new Promise((resolve, reject)=>{
			...
			var url_prefix = (spec.apiVersion == "apps/v1")? K8S_APPS_V1_API : K8S_V1_API;
			
			var options = {
				url: url_prefix+'/namespaces'+((kind == 'namespaces')? '': ('/'+ns+'/'+kind)),
				method: 'POST',
				headers: {
					"Authorization": "Bearer "+API_TOKEN,
					"Content-Type": "application/json"
			...
			if (kind == 'pods')
				...
				create_deploy(spec, options, callback);
				...
				create_secret(spec, options, callback);
			else if (kind == 'persistentvolumes')
				create_pv(spec, options, callback);
			else if (kind == 'persistentvolumeclaims')
				create_pvc(spec, options, callback);
			else
				callback(new Error("Don't know how to contruct this object"));
		});
	}
	...
	var _delete = (spec)=>{
		return new Promise((resolve, reject)=>{
			...
			var options = {
				url: K8S_V1_API+'/namespaces/'+ns+((kind == 'namespaces')? '': ('/'+kind+'/'+spec.metadata.name))+'?pretty=true',
				method: 'DELETE',
				headers: {
					"Authorization": "Bearer "+API_TOKEN,
					"Content-Type": "application/json"
			...
			if (kind == 'pods')
				...
				delete_deploy(spec, options, callback);
			else if (kind == 'replicationcontrollers')
				...
				delete_secret(spec, options, callback);
			else if (kind == 'persistentvolumes')
				delete_pv(spec, options, callback);
			else if (kind == 'persistentvolumeclaims')
				delete_pvc(spec, options, callback);
			else
				callback(new Error("Don't know how to delete this object"));
		});
	}
	...
	var _patch = (spec)=>{
		return new Promise((resolve, reject)=>{
			...
			request(options, (error, response, body)=>{
				// console.log("error : "+error);
				console.log(`PATCH ${spec.kind} ${spec.metadata.name}, Status: ${response.statusCode}`);
				// console.log("body : "+JSON.stringify(JSON.parse(body)['spec']) );
				if (error || (response.statusCode != 200 && response.statusCode != 201))
					return callback(error || JSON.parse(body).message)
...
ns_skante.create({
	...
	ns_skante.patch({
		...
		console.log("BEFORE : "+JSON.stringify(out1, null, 2));
		console.log("-------------------------------------------");
		console.log("AFTER : "+JSON.stringify(out2, null, 2));

	}).catch(function(reason){ console.log("Failure replacing: "+reason); });;
}).catch(function(reason){ console.log("Failure : "+reason); });
*/
